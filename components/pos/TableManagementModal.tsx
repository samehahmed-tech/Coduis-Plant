...
const TableManagementModal: React.FC<TableManagementModalProps> = ({
    ...
    return (
        <AnimatePresence>
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 theme-modal-overlay z-[300] flex items-end sm:items-center justify-center p-0 sm:p-4">
                <div className="absolute inset-0" onClick={onClose} />
                <motion.div
                    initial={{ y: "100%", scale: 1 }} animate={{ y: 0, scale: 1 }} exit={{ y: "100%", scale: 1 }} transition={{ type: "spring", damping: 26, stiffness: 300 }}
                    className={`theme-modal-content w-full max-w-2xl relative overflow-hidden flex flex-col max-h-[90vh] sm:max-h-[85vh] ${isRTL ? 'text-right' : 'text-left'}`}
                >
                    {renderHeader()}
