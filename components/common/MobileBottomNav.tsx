...
const MobileBottomNav: React.FC<MobileBottomNavProps> = ({ onOpenCommand }) => {
    ...
    const { settings, toggleSidebar, hasPermission } = useAuthStore();
    const lang = settings.language || 'en';
    const navItems = PRIMARY_MOBILE_NAV.filter(i => hasPermission(i.permission));
        ...
        <nav className="workspace-mobile-nav fixed bottom-0 left-0 right-0 z-50 lg:hidden">
            <div className="workspace-mobile-nav-inner flex items-center justify-around px-2 pt-2 pb-1">
                ...
                <button onClick={onOpenCommand}
                    ...
                    <span className="text-[9px] font-bold uppercase tracking-wider">{lang === 'ar' ? 'بحث' : 'Search'}</span>
                </button>
                <button onClick={() => {
                    ...
                    <span className="text-[9px] font-bold uppercase tracking-wider text-muted/60">{lang === 'ar' ? 'المزيد' : 'More'}</span>
                </button>
            </div>
        </nav>
    );
