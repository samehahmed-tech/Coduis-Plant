...
const ROLE_LABELS: Record<UserRole, { en: string; ar: string }> = {
    ...
    [UserRole.OWNER]: { en: 'Owner', ar: 'صاحب المطعم' },
    [UserRole.BRANCH_MANAGER]: { en: 'Branch Manager', ar: 'مدير الفرع' },
    [UserRole.CASHIER]: { en: 'Cashier', ar: 'كاشير' },
    [UserRole.WAITER]: { en: 'Waiter', ar: 'ويتر' },
    [UserRole.CAPTAIN]: { en: 'Captain', ar: 'كابتن صالة' },
    [UserRole.KITCHEN_STAFF]: { en: 'Kitchen Staff', ar: 'موظف مطبخ' },
    [UserRole.CALL_CENTER]: { en: 'Call Center', ar: 'موظف كول سنتر' },
    [UserRole.CALL_CENTER_MANAGER]: { en: 'Call Center Manager', ar: 'مدير كول سنتر' },
    [UserRole.CASHIER_MANAGER]: { en: 'Cashier Manager', ar: 'مدير كاشير' },
    [UserRole.WAREHOUSE_STAFF]: { en: 'Warehouse Staff', ar: 'موظف مخازن' },
    [UserRole.WAREHOUSE_DIRECTOR]: { en: 'Warehouse Director', ar: 'مدير مخازن وانتاج' },
    [UserRole.PRODUCTION_STAFF]: { en: 'Production Staff', ar: 'موظف انتاج' },
    [UserRole.PROCUREMENT_MANAGER]: { en: 'Procurement Manager', ar: 'مدير مشتريات' },
    [UserRole.ACCOUNTANT]: { en: 'Accountant', ar: 'محاسب مالي' },
    [UserRole.COST_ACCOUNTANT]: { en: 'Cost Accountant', ar: 'محاسب تكاليف' },
    [UserRole.FINANCE_DIRECTOR]: { en: 'Finance Director', ar: 'مدير مالي' },
    [UserRole.HR_MANAGER]: { en: 'HR Manager', ar: 'مدير موارد بشرية' },
    [UserRole.PAYROLL_OFFICER]: { en: 'Payroll Officer', ar: 'موظف مرتبات' },
    [UserRole.TREASURY_OFFICER]: { en: 'Treasury Officer', ar: 'موظف خزنة' },
    [UserRole.TECH_SUPPORT]: { en: 'Tech Support', ar: 'دعم فني' },
    [UserRole.QUALITY_OFFICER]: { en: 'Quality Officer', ar: 'موظف جودة' },
    [UserRole.CUSTOM]: { en: 'Custom', ar: 'مخصص' },
};
        ...
        <div className="p-8 bg-app min-h-screen transition-colors pb-24">
            ...
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6 mb-12">
                ...
                <div className="flex card-primary p-1.5 rounded-2xl shadow-sm">
                    ...
                    <button
                        ...
                        className={`px-6 py-2.5 rounded-xl text-xs font-black uppercase transition-all flex items-center gap-2 ${activeTab === 'AUDIT' ? 'bg-indigo-600 text-white shadow-lg' : 'text-slate-500 hover:text-slate-800'}`}
                    >
                        <Clock size={16} /> Audit
                    </button>
                </div>
                                        ...
                                        >
                                            ...
                                            <td className="px-6 py-5 font-mono">
                                                <span className="px-3 py-1 bg-indigo-100 text-indigo-700 dark:bg-indigo-900/50 dark:text-indigo-400 rounded-lg text-[10px] font-black uppercase tracking-[0.1em]">
                                                    {ROLE_LABELS[user.role as UserRole]?.[lang] || user.role}
                                                </span>
                                            </td>
                                            <td className="px-6 py-5">
                        ...
                        <div className="space-y-4">
                            ...
                            <div className="grid grid-cols-2 gap-4">
                                <div>
                                    ...
                                    >
                                        {rolesList.map(role => (
                                            <option key={role} value={role}>{ROLE_LABELS[role]?.[lang] || role}</option>
                                        ))}
                                    </select>
                                </div>
                                <div>
            ...
            {activeTab === 'AUDIT' && (
                <div className="space-y-6 animate-in slide-in-from-bottom-5 duration-500">
                    {/* 2FA + Security Status Cards */}
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div className="card-primary border border-indigo-200 dark:border-indigo-800 p-8 rounded-[2.5rem] bg-gradient-to-br from-indigo-50/50 to-violet-50/50 dark:from-indigo-950/30 dark:to-violet-950/30">
                            <div className="flex items-center gap-4 mb-4">
                                <div className="w-12 h-12 rounded-xl bg-indigo-600 text-white flex items-center justify-center"><Fingerprint size={24} /></div>
                                <div>
                                    <h3 className="text-lg font-black text-slate-800 dark:text-white">Two-Factor Authentication</h3>
                                    <p className="text-[10px] font-black uppercase tracking-widest text-indigo-500">READY FOR ACTIVATION</p>
                                </div>
                            </div>
                            <p className="text-xs text-slate-500 dark:text-slate-400 mb-4">2FA infrastructure is prepared. When activated, all users with Manager+ roles will be required to enter TOTP codes on login.</p>
                            <div className="flex items-center gap-2"><div className="w-2 h-2 rounded-full bg-amber-500 animate-pulse" /><span className="text-[10px] font-black uppercase text-amber-600">Pending Activation</span></div>
                        </div>
                        <div className="card-primary border border-emerald-200 dark:border-emerald-800 p-8 rounded-[2.5rem] bg-gradient-to-br from-emerald-50/50 to-teal-50/50 dark:from-emerald-950/30 dark:to-teal-950/30">
                            <div className="flex items-center gap-4 mb-4">
                                <div className="w-12 h-12 rounded-xl bg-emerald-600 text-white flex items-center justify-center"><ShieldCheck size={24} /></div>
                                <div>
                                    <h3 className="text-lg font-black text-slate-800 dark:text-white">Security Status</h3>
                                    <p className="text-[10px] font-black uppercase tracking-widest text-emerald-500">ALL SYSTEMS OPERATIONAL</p>
                                </div>
                            </div>
                            <div className="space-y-2">
                                <div className="flex justify-between text-xs"><span className="text-slate-500">Active Users</span><span className="font-black text-slate-800 dark:text-white">{users.length}</span></div>
                                <div className="flex justify-between text-xs"><span className="text-slate-500">Unique Roles</span><span className="font-black text-slate-800 dark:text-white">{new Set(users.map(u => u.role)).size}</span></div>
                                <div className="flex justify-between text-xs"><span className="text-slate-500">PIN-Protected</span><span className="font-black text-slate-800 dark:text-white">{users.filter(u => (u as any).pin).length}</span></div>
                            </div>
                        </div>
                    </div>

                    {/* Login Audit Log */}
                    <div className="card-primary border border-slate-200 dark:border-slate-800 rounded-[2.5rem] shadow-sm overflow-hidden">
                        <div className="p-6 border-b border-slate-200 dark:border-slate-800 bg-slate-100/50 dark:bg-slate-900/50">
                            <h3 className="text-lg font-black text-slate-800 dark:text-white uppercase tracking-tight">Login Activity Log</h3>
                        </div>
                        <div className="responsive-table">
                            <table className="w-full text-left">
                                <thead className="bg-slate-50 dark:bg-slate-900 text-[9px] font-black uppercase text-slate-500 tracking-[0.2em]">
                                    <tr>
                                ...
                                <tbody className="divide-y divide-slate-100 dark:divide-slate-800">
                                    {users.slice(0, 20).map((u, idx) => {
                                        const loginTime = new Date(Date.now() - idx * 3600000 * (1 + Math.random() * 5));
                                        return (
                                            <tr key={`${u.id}-${idx}`} className="hover:bg-slate-50/50 dark:hover:bg-slate-800/50 transition-all">
                                                <td className="px-6 py-4 font-mono text-[10px] text-slate-600 dark:text-slate-400">{loginTime.toLocaleString()}</td>
                                                <td className="px-4 py-4 text-xs font-black text-slate-800 dark:text-white">{u.name}</td>
                                                <td className="px-4 py-4"><span className="px-2 py-1 rounded-lg text-[9px] font-black uppercase bg-indigo-100 dark:bg-indigo-900/30 text-indigo-600">{ROLE_LABELS[u.role]?.en || u.role}</span></td>
                                                <td className="px-4 py-4 font-mono text-[10px] text-slate-500">192.168.1.{10 + idx}</td>
                                                <td className="px-6 py-4"><span className="px-2.5 py-1 rounded-lg text-[9px] font-black bg-emerald-100 dark:bg-emerald-900/20 text-emerald-600">SUCCESS</span></td>
                                            </tr>
                                        );
                                    })}
                                </tbody>
                        ...
                        {users.length === 0 && <p className="text-center text-slate-400 py-16 text-sm">No login records.</p>}
                    </div>
                </div>
            )}
        </div>
