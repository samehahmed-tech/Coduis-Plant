  ...
  return (
    <div className="p-8 bg-slate-50 dark:bg-slate-950 min-h-screen transition-colors">
      <div className="flex justify-between items-center mb-8">
        <div>
          <h2 className="text-3xl font-bold text-slate-800 dark:text-white">Inventory & Supply Chain</h2>
          <p className="text-slate-500 dark:text-slate-400">Manage stocks, suppliers, and purchase orders.</p>
        </div>
        <button className="flex items-center gap-2 bg-indigo-600 text-white px-4 py-2 rounded-lg hover:bg-indigo-700 transition-colors shadow-sm font-bold">
          <Plus size={20} />
          {activeTab === 'STOCK' ? 'Add Item' : activeTab === 'SUPPLIERS' ? 'Add Supplier' : 'Create PO'}
        </button>
      </div>
      ...
      <div className="flex justify-between items-center mb-6">
        <div className="flex gap-4 border-b border-slate-200 dark:border-slate-800">
          <button 
            ...
            className={`pb-3 px-2 font-medium flex items-center gap-2 transition-colors border-b-2 ${activeTab === 'STOCK' ? 'border-indigo-600 text-indigo-600' : 'border-transparent text-slate-500 hover:text-slate-700'}`}
          >
            <Package size={18} /> Stock Items
          </button>
          <button 
            onClick={() => setActiveTab('SUPPLIERS')}
            className={`pb-3 px-2 font-medium flex items-center gap-2 transition-colors border-b-2 ${activeTab === 'SUPPLIERS' ? 'border-indigo-600 text-indigo-600' : 'border-transparent text-slate-500 hover:text-slate-700'}`}
          >
            <Truck size={18} /> Suppliers
          </button>
          <button 
            onClick={() => setActiveTab('PO')}
            className={`pb-3 px-2 font-medium flex items-center gap-2 transition-colors border-b-2 ${activeTab === 'PO' ? 'border-indigo-600 text-indigo-600' : 'border-transparent text-slate-500 hover:text-slate-700'}`}
          >
        ...
        <div className="relative w-64">
          ...
          <input 
            ...
            className="w-full pl-10 pr-4 py-2 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-lg outline-none focus:ring-2 focus:ring-indigo-500 transition-all shadow-sm"
          />
        </div>
      </div>
          ...
          <div className="bg-white dark:bg-slate-900 w-full max-w-2xl rounded-2xl shadow-2xl overflow-hidden animate-in zoom-in-95 duration-200">
            <div className="p-6 border-b border-slate-200 dark:border-slate-800 flex justify-between items-center bg-slate-50 dark:bg-slate-950/50">
               <div className="flex items-center gap-3">
                  ...
                  <div>
                    <h3 className="text-xl font-black text-slate-800 dark:text-white">{selectedSupplier.name}</h3>
                    <p className="text-xs font-bold text-slate-500 uppercase tracking-widest">{selectedSupplier.category} Supplier</p>
                  </div>
               </div>
               <button onClick={() => setSelectedSupplier(null)} className="text-slate-400 hover:text-slate-600 dark:hover:text-slate-200 p-2 hover:bg-slate-200 dark:hover:bg-slate-800 rounded-full transition-colors">
                 <X size={24} />
               </button>
            </div>
            ...
            <div className="p-8">
               ...
               <div className="flex gap-4">
                  <button className="flex-1 py-4 bg-indigo-600 text-white rounded-xl font-bold hover:bg-indigo-700 shadow-lg shadow-indigo-200 dark:shadow-none transition-all flex items-center justify-center gap-2">
                    <FileText size={20} /> Create Purchase Order
                  </button>
                  <button onClick={() => setSelectedSupplier(null)} className="flex-1 py-4 bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-300 rounded-xl font-bold hover:bg-slate-200 dark:hover:bg-slate-700 transition-all">
                    Close Details
                  </button>
               </div>
            </div>
