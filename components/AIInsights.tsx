    ...
    const runAnalysis = async () => {
        ...
        try {
            const [menuRes, invRes] = await Promise.all([
                aiApi.chat({
                    message: lang === 'ar' ? 'اعمل تحليل هندسة منيو مختصر من 3 نقاط عملية.' : 'Provide a short menu engineering analysis in 3 practical points.',
                    lang,
                    context: { menuItems, orders, categories, inventory },
                }),
                aiApi.chat({
                    message: lang === 'ar' ? 'اعمل توقع مخزون مختصر مع العناصر المعرضة للنفاد واقتراحات إعادة الطلب.' : 'Provide a short inventory forecast with at-risk items and restock suggestions.',
                    lang,
                    context: { inventory, orders, menuItems, categories },
                }),
            ]);
        ...
        <div className="p-8 bg-slate-50 dark:bg-slate-950 min-h-screen transition-colors pb-24">
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6 mb-12">
                <div>
                    ...
                    <p className="text-slate-500 dark:text-slate-400 font-semibold italic">Smart analysis of your operations, sales, and inventory.</p>
                </div>
                <button
                    onClick={runAnalysis}
                ...
                >
                    ...
                    {isAnalyzing ? (lang === 'ar' ? 'جاري التحليل...' : 'Analyzing System...') : (lang === 'ar' ? 'تحديث التحليل' : 'Re-Analyze')}
                </button>
            </div>
            ...
            <div className="mt-10 p-10 bg-indigo-600 rounded-[3.5rem] text-white flex flex-col md:flex-row items-center justify-between gap-8 shadow-2xl shadow-indigo-600/40 relative overflow-hidden group">
                ...
                <div className="relative">
                    <h3 className="text-3xl font-black uppercase tracking-tighter mb-2">AI Analysis Report</h3>
