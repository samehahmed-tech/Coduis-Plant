...
export const barcodeApi = {
    /** Unified lookup — searches both menu and inventory items by barcode/SKU */
    lookup: (code: string) =>
        apiRequest<{
            found: boolean;
            code?: string;
    ...
    /** Check if barcode is already used (for registration validation) */
    ...
    /** Generate a unique random EAN-13 barcode */
    generate: () =>
...
export const reportsApi = {
    ...
    getReorderAlerts: () => {
        return apiRequest<Array<{ itemId: string; itemName: string; unit: string; threshold: number; costPrice: number; currentStock: number; deficit: number }>>(`/reports/reorder-alerts`);
    },
    getExpiringBatches: () => {
        return apiRequest<{ items: Array<{ batchId: string; batchNumber: string; itemName: string; unit: string; currentQty: number; unitCost: number; expiryDate: string; receivedDate: string; status: string }>; totalAtRiskValue: number; alreadyExpired: number; totalBatches: number }>(`/reports/expiring-batches`);
    },
    ...
    getCampaignROI: () => {
        return apiRequest<Array<{ id: string; name: string; type: string; status: string; reach: number; conversions: number; revenue: number; budget: number; roi: number; conversionRate: number; createdAt: string }>>(`/reports/campaign-roi`);
    },
    ...
    getInventoryValuation: () => {
        return apiRequest<{ totalValue: number; items: Array<{ itemId: string; itemName: string; unit: string; totalQty: number; totalValue: number; avgUnitCost: number; batchCount: number }> }>(`/reports/inventory-valuation`);
    },
    ...
    getRecipeCostAlerts: () => {
        return apiRequest<{ critical: number; warning: number; ok: number; items: any[] }>(`/reports/recipe-cost-alerts`);
    },
    getABCClassification: () => {
        return apiRequest<{ totalValue: number; a: number; b: number; c: number; items: any[] }>(`/reports/abc-classification`);
    },
    ...
    getLoyaltyPoints: () => {
        return apiRequest<{ summary: any; tiers: any[]; topPointHolders: any[] }>(`/reports/loyalty-points`);
    },
    ...
    getCustomerJourney: () => {
        return apiRequest<any[]>(`/reports/customer-journey`);
    },
    ...
    getOptimalPricing: () => {
        return apiRequest<{ targetMargin: number; items: any[]; needsIncrease: number; ok: number; canDecrease: number }>(`/reports/optimal-pricing`);
    },
    ...
    getTimeToFirstOrder: () => {
        return apiRequest<any[]>(`/reports/time-to-first-order`);
    },
};
